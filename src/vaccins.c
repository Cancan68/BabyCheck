 ...
 * Returns: Nothing
 */
Int16 VaccinsCompare(ShotsDBType *r1, ShotsDBType *r2, Int16 sortOrder, SortRecordInfoPtr info1, SortRecordInfoPtr info2, MemHandle appInfoH)
{
  Int16 result;
 ...
 * Returns: Nothing
 */
void VaccinsDrawDate(void *table, Int16 row, Int16 column, RectanglePtr bounds)
{
  DateType date;
...
void VaccinsLoadTable(void)
{
  ...
  table = (TablePtr) FrmGetObjectPtr(form, FrmGetObjectIndex(form, tableID_vaccins));
  ...
  nbrec = 1;
  dataHeight = 0;
  recNum = DmNumRecords(ShotsDBRef);
  rec = 0;

  while (rec < recNum)
  {
    h = DmQueryRecord(ShotsDBRef, (UInt16) rec);
    if (h)
    {
